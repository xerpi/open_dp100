...
use crate::{
    data::{
        BasicInfo, BasicSet, DeviceInfo, OpResult, OperationResult, OutputState, ScanOut,
        SerialOut, SystemInfo,
    },
    error::OpenDP100Error,
    opcode::OpCode,
};
use endianness::{read_i16, read_u16, ByteOrder};
use std::convert::TryInto;

const MAX_FRAME_DATA_LEN: usize = 64 - 4 - 2;
...
impl Frame {
    ...
    pub fn new(op_code: OpCode, data: &[u8]) -> Self {
        ...
        assert!(data.len() <= MAX_FRAME_DATA_LEN);
    ...
    pub fn append_data(&mut self, data: &[u8]) -> Result<(), OpenDP100Error> {
        if data.len() + self.op_data_len > MAX_FRAME_DATA_LEN {
