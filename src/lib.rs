use frame::{deserialize_in_frame, serialize_out_frame, Frame, FrameError, Operational};
use hidapi::{HidApi, HidDevice};

pub use error::OpenDP100Error;
pub use opcode::OpCode;

pub use data::{BasicInfo, BasicSet, DeviceInfo, OperationResult, OutputState, SystemInfo};
...
enum BasicSetCommand {
    SwitchOutput,
    ModifyProfile,
    ActivateProfile,
}

impl BasicSetCommand {
    fn flag(&self) -> u8 {
        match self {
            BasicSetCommand::SwitchOutput => 0x20,
            BasicSetCommand::ModifyProfile => 0x40,
            BasicSetCommand::ActivateProfile => 0xa0,
        }
    }
}
...
macro_rules! session_impl {
    ($self:expr,$op_code:pat,$req:expr,$res:ident) => {{
        ...
        match response_res {
            ...
            Err(_) => Err(OpenDP100Error::DEVICE),
        }
    }};
}
...
impl OpenDP100 {
    ...
    fn update_basic_set_with_command(
        &self,
        set_req: &BasicSet,
        command: BasicSetCommand,
    ) -> Result<(), OpenDP100Error> {
        if set_req.index >= 10 {
            return Err(OpenDP100Error::INVALID_PARAM);
        }

        let mut set = (*set_req).clone();
        set.index = command.flag() + set.index;

        let req: Frame = Frame::new(OpCode::BasicSet, &set.to_data());
        let r = session_impl!(self, OpCode::BasicSet, &req, OperationResult);

        match r {
            Ok(ok) => match ok.result {
                data::OpResult::Success => Ok(()),
                _ => Err(OpenDP100Error::DEVICE),
            },
            Err(e) => Err(e),
        }
    }

    pub fn update_basic_set(&self, set_req: &BasicSet) -> Result<(), OpenDP100Error> {
        self.update_basic_set_with_command(set_req, BasicSetCommand::ModifyProfile)
    }
}
...
impl OpenDP100 {
    pub fn set_output_on(&self, on: OutputState) -> Result<(), OpenDP100Error> {
        ...
        basic_set.state = on;
        self.update_basic_set_with_command(&basic_set, BasicSetCommand::SwitchOutput)?;
        Ok(())
    }

    pub fn switch_config(&self, idx: usize) -> Result<(), OpenDP100Error> {
        let config_set = self.basic_set(idx)?;
        self.update_basic_set_with_command(&config_set, BasicSetCommand::ActivateProfile)?;
        Ok(())
    }
}
